...
SERVER_PROTOCOL=`sed '/dubbo.protocols/!d;s/.*=//;s/:.*//' conf/config.properties | tr -d '\r'`
SERVER_PORT=`sed '/dubbo.protocols/!d;s/.*=//;s/.*://' conf/config.properties | tr -d '\r'`
LOGS_FILE=`sed '/logback.file.path/!d;s/.*=//' conf/config.properties | tr -d '\r'`
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $JAVA_GC_OPTS -classpath $CONF_DIR:$LIB_JARS:$PAY1PAYLIB_JARS com.tota.se.core.container.DefaultContainer &>/dev/null &
